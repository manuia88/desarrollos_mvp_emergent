...
| ~~W4.13.B~~ | ~~Project Performance Intelligence (Meta Ads automation)~~ → **MOVIDO Wave 5 W5.10** | 20 | — | — | 📦 W5 | — | módulo entero Social/Ads Multi-tenant + Analytics Granular + IA Layer pasa a W5.10 (205h) · founder ops Meta App Review en paralelo H1 |
...
| F0 | Sweep tech debt + **Notification Center bell icon (G2 ✅ shipped W4.17)** + Apify token rotation + tier `whatif_simulator`/`asistente_publico` seed + 21.5h enhancements priorizados | 64 | 14 | mixto | 🟡 | F0.1 `cfbf5ab` + F0.2 `834eb27` + F0.3-PARTIAL CSV export only | F0.1 ✅ + F0.2 ✅ + F0.3-PARTIAL ✅ (CSV export Free Audit) · ⚠️F0.3 refactor 125 archivos `routes_*.py` → `routes/{name}.py` + CLI lead_nurture --dry-run DIFERIDOS a F0.4 manual sesión dedicada (cherry-pick masivo riesgoso · sitemap .com violation + .gitignore inflado · branch atrasado destructivo) · 50h pending F0.4 |
