...
| F0 | Sweep tech debt + **Notification Center bell icon (G2 ✅ shipped W4.17)** + Apify token rotation + tier `whatif_simulator`/`asistente_publico` seed + 21.5h enhancements priorizados (Score Inversión + AVM hedonic swap + Resend welcome + rate-limit waitlist + digest semanal + leaderboard + brochure regenerate + 3DGS thumbnail + Free Audit funnel + refactor routes + CLI dry-run + Microsoft OAuth) | 64 | 6.5 | mixto | 🟡 | F0.1 `cfbf5ab` (Score Inversión + AVM hedonic swap + Resend welcome + rate-limit waitlist) | F0.1 ✅ 6.5h shipped · 57.5h pending split en F0.2/F0.3/F0.4 |
