...
| **Valores unitarios 2026** ✅ stub | Tablas oficiales $/m² suelo + construcción por zona (Gaceta CDMX · Código Fiscal 2026) | **Conector LISTO** (`valores_unitarios_engine` · status + ingest + botón superadmin) · honesto no-op hasta cargar la tabla · al cargar, prefiere este valor sobre el SIG 2022. **Links 2026 localizados** (Finanzas CDMX · `transparencia.finanzas.cdmx.gob.mx/.../valores_unitarios_cfcdmx_2026_parte1.pdf` + `_parte2.pdf`) · 📦 OCR DIFERIDO (PDF escaneado · ver BACKLOG_ENHANCEMENTS) | `IE_VALORES_UNITARIOS_2026_URL` | ancla de suelo MÁS reciente (refuerzo de ING.2; opcional porque el modelo ya aprende de ventas) |
| **SHF Índice precios vivienda** ✅ | Plusvalía OFICIAL **por alcaldía** + serie 2005–2026 | **HECHO** (`shf_engine` · snapshot 1T2026 + serie 510 trim. en `shf_series` desde `backend/data/shf_cdmx_serie.csv` + parser XLSX real verificado) | `IE_SHF_XLSX_URL` (default gob.mx) | plusvalía por alcaldía (5 propias, resto hereda CDMX +4.5%) + gráfica de historia · en valuación del comprador |

### ⚠️ CORRECCIÓN CLAVE (reporte verificado founder 2026-06-07)
- **SHF NO existe en el SIE de Banxico** (cualquier serie ID es inventada). Solo XLSX en gob.mx (trimestral feb/may/ago/nov · 10,285 filas · estructura Consecutivo|Global|Estado|Municipio|Trimestre|Año|Indice · serie 2005–2026). 1T2026 nacional +8.7% · nueva +9.1%/usada +8.3% · avalúo mediana $1,331,000. XLSX: `gob.mx/cms/uploads/attachment/file/1077618/Indice_SHF_datos_abiertos_1_trim_2026.xlsx`.
- **GRANULARIDAD CDMX (verificado XLSX descargado)**: SOLO **5 alcaldías** tienen índice SHF propio (volumen hipotecario suficiente): Cuauhtémoc 185.20 (+4.7%) · Miguel Hidalgo 180.01 (+4.4%) · Benito Juárez 179.68 (+4.5%) · Iztapalapa 173.12 (+5.2%) · Gustavo A. Madero 171.30 (+4.9%). **CDMX estatal 175.20 (+4.5%)** → lo heredan las otras 11 alcaldías. ⚠️ CORRECCIÓN: el **+5.1% del boletín es la ZM Valle de México (INCLUYE Edomex)**, NO la CDMX como entidad (+4.5%). Base 2005≈33 → nivel ~175 = precio nominal ~5× desde 2005. Implementado en `shf_engine` (snapshot por alcaldía + `get_appreciation(alcaldia)` propio/heredado + serie histórica para gráfica). Para las 11 sin índice: factor de plusvalía = CDMX estatal, nivel absoluto se diferencia con el valor catastral del suelo (SIG · ING.1).
...
- ✅ **SHF plusvalía oficial POR ALCALDÍA** + serie histórica 2005–2026 (`shf_engine` · 510 trimestres en `shf_series` · parser XLSX real verificado contra el archivo oficial · botón superadmin "Refrescar Plusvalía (SHF)" + tabla 5 alcaldías · en valuación del comprador con gráfica de tendencia). Corrige el +5.1% (era ZM, no CDMX) → CDMX +4.5%.
