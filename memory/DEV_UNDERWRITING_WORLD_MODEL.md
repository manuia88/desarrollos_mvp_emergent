...
**F1.4 ✅ HECHO: Detector Norma 3 (fusión de predios rentable)** — dada la colonia del predio, busca colonias VECINAS (cercanía de centroide, radio 1.5 km) con CUS más alto y cuantifica el uplift en valor de terreno si capturas ese CUS (mismo lugar/precio, solo cambia lo construible). Reutiliza `valor_residual_engine` (mismo cálculo F1.2) + colonias (center+cus, F1.0). Ordena por $ en juego. Backend `norma3_engine.py` + endpoint `/api/dev/valor-residual/norma3`. Sección integrada en la pantalla Valor de Terreno. Honesto: señal a nivel zona, NO confirma colindancia legal predio-a-predio (lo verifica perito/SEDUVI). Verificado live (Tetelpan CUS 1.2→2.1 = +$7.2M/+75%).

**F1.1 ✅ HECHO: Doctrina de Datos visible (cross-portal)** — fuente ÚNICA `backend/data_doctrine.py`: 5 tipos de origen (dato/benchmark/cálculo/estimado/supuesto, con confianza+color+definición simple) + las 7 reglas inviolables + 3 niveles de madurez (experimental/validado/robusto) + helper `tag()` para que los motores emitan orígenes consistentes. Endpoint cross-portal `/api/doctrine` (cualquier rol). Componente compartido `components/shared/DataOrigin.js`: badge `<DataOrigin>` + `<DoctrineButton>` ("Cómo leemos los datos" → modal explicativo). Cableado en Valor de Terreno (reemplaza el tag local) + disponible para los 4 portales. Las bandas honestas siguen en `metric_normalizer` (no se duplica). Verificado live.

**F1.5 ✅ HECHO: 1-pantalla integrada + Veredicto del lote** — el asistente "lee" el lote completo en UNA acción: orquesta los 3 motores (residual+DD+Norma3) y los sintetiza en un VEREDICTO claro ("Paga hasta $X · margen sano · N cosas que revisar · +$Y si fusionas") + 4 bullets (Precio/Revisión/Oportunidad/Confianza). Backend `lote_veredicto_engine.py` + endpoint `/api/dev/valor-residual/analizar` (UNA llamada en vez de 3, fail-open). Frontend: ribbon "Lo que vi en este lote" arriba (respuesta primero) + los detalles (desglose/DD/Norma3) colapsados por defecto (detalle bajo demanda · principio UX/UI). Verificado live.

**F1.6 ✅ HECHO: Calibración formal vs Puente Alvarado — 🎉 F1 COMPLETA** — el "examen" (Doctrina regla #6): mete los datos reales del caso golden a nuestras fórmulas del terreno y compara predicción↔realidad. 5 chequeos (margen ✓ · comisión+publicidad · honorarios fee+gerencia · costo obra/m² vs Neodata ✓ · incidencia del terreno) cada uno calibrado/ajustar + diferencia %. Rulings founder 2026-06-09: comisión 2% (estándar CDMX) y honorarios de desarrollo 16% (fee 10%+gerencia 6%, referencia metodología) — AMBOS editables por el dev en «Ajustes finos», sin estándar único impuesto. La calibración los muestra como REFERENCIA (info), no pass/fail. Examen global = calibrado (margen ✓ · comisión ✓ · obra/m² ✓; honorarios e incidencia = referencia). Cierra el ciclo: botón "Aplicar valores calibrados" escribe `db.calibracion_terreno`; el motor residual lo lee vía `get_effective_defaults()` (verificado: margen pasa a 0.184 en vivo). Backend `golden_calibration_engine.py` + 2 endpoints superadmin. Frontend `SuperadminCalibracion.js` (nav Inteligencia → Calibración). Alcance F1=terreno; TIR/flujo se calibra en F2/F3.

🎉 **FASE F1 (Valor Residual + Due Diligence + Norma 3 + Doctrina visible + 1-pantalla + Calibración) COMPLETA Y VERIFICADA.** Siguiente: F2 Autopiloto de Memorándum.
