...
| EX.2b | **Ingesta DENUE real por zona**: arreglado el método del API (`BuscarEntorno`→`Buscar`, endpoint muerto) + bug del centro genérico (ahora cada colonia usa SU centro real, verificado coords distintas) + orquestador `sync_denue_for_city` + botón "Sincronizar Negocios (DENUE)" + recálculo en un clic + no-op honesto. **Falta config/prod:** verificar `IE_DENUE_TOKEN` en INEGI + que el API sea alcanzable desde el server (no verificable desde el sandbox) | 🟡 pipe hecho (2026-06-07) · espera token/prod |
| EX.2c | Ingesta de **crimen** (SESNSP CSV → `crime_data_sesnsp` + población en `dim_zones`) o FGJ-por-colonia (resource 48fcb848) — seguridad real | ⏳ siguiente ingesta |
