...
**📍 DÓNDE ESTAMOS:** capa de datos cerrada (Tandas A/B/C ✅ · B.2 100% incl. fit/match · EX 7/7 dims · 1,352 llenándose). Ingesta oficial: SHF por alcaldía ✅ + SIG valor catastral del suelo $/m² ✅ + modelo suelo→comercial ✅ (flywheel · stub honesto). Portal Dev (Paso C ✅ + dato falso→real ✅ + **aislamiento cross-org ✅**: candado central `dev_guard` + bitácora + Centro de Seguridad superadmin + red-team 16/16). **QA del Portal Dev ✅ Ola 1 (5 áreas usuario) + Ola 2 (técnico: seguridad/IDOR, arranque, atomicidad/CAS, 4 portales) + cola bajo riesgo + Cross-Portal v2 ✅ (asesor ve dato fresco · fotos dev→comprador con candado · amenidades+foto en listados sin frenar).** Scorecard en `memory/DEV_QA_SCORECARD.md`. **Pendiente real:** Bloque 4/5 (prender llaves + abrir a usuarios reales) — el portal del dev está completo, honesto, seguro, sin "/100", móvil y con los 4 portales conectados. Cero deuda: ING.2b conector stub listo (OCR tabla 2026 en backlog).
...
| Ingesta oficial — **SHF** (plusvalía) ✅ · **SIG → valor catastral del suelo $/m²** ✅ · **modelo suelo→comercial** (regresión que aprende de ventas · flywheel) ✅ motor+UI · stub honesto | ✅ SHF · ✅ SIG · ✅ modelo (en stub, se enciende con ventas) |
...
| **QA del Portal Dev** (Ola 1 usuario + Ola 2 técnico + cola + Cross-Portal v2) | ✅ Ola 1 (B·C·A·D·F) + Ola 2 (seguridad/IDOR·arranque·atomicidad·4 portales) + cola bajo riesgo + **Cross-Portal v2 ✅** (#15 asesor ve dato fresco · #16 fotos dev→comprador con candado · #17 amenidades+foto en listados sin frenar). Scorecard: `memory/DEV_QA_SCORECARD.md` |
...
| EX.2 | Scores REALES por colonia: **puente `score_bridge` + cómputo por catálogo + cobertura** ✅. **7 de 7 dimensiones reales** ✅: vida·comercio·movilidad·educación (OSM) + seguridad·riesgo (FGJ) + **plusvalía/precio (valuación 4-fuentes por percentil · `precio_rescore` en el cron)**. Verificado: Polanco 7/7 (plusvalía score 97 = zona de mayor valor). El precio es real donde hay cierres/reventa/obra (estimado/stub donde no). | ✅ 7/7 dims (2026-06-08) |
