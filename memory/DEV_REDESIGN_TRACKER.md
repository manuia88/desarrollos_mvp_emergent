...
- ✅ **C.6 Marketing (2026-06-06, Bloque 1.6):** el Marketing del dev ya tenía Centro de Marketing (Mini Market + launcher de Studio: landings/carruseles/video/auto-content). GAP IA-first = qué promocionar. NUEVO **"Qué Promocionar Hoy"** (DevMarketingJugadas.js, tema claro, arriba del Mini Market): por proyecto recomienda el CONTENIDO según su salud (estancado→Landing · demanda tibia→Carruseles · interés caliente→Video · buen ritmo→Auto-Content) con CTA al Studio. Reusa _stock_soldout(dev_ids)+leads. Back: GET /api/desarrollador/marketing-jugadas. Cierra ciclo señal→contenido. Verificado en vivo.
- ✅ **C.5 Red Comercial (2026-06-06, Bloque 1.5):** el dev ya tenía directorio rico (inmobiliarias/in-house/freelance + métricas equipo/asesores + drawer + strip solicitudes/disputas). GAP IA-first = síntesis de salud. NUEVO **"Salud de tu Red"** (DevRedSalud.js, tema claro, arriba del directorio): ranking de asesores (leads/won/conversión/canal) + concentración + split in-house/broker + estrella + quién no cierra + acciones agentic (diversifica/reasigna/suma-brokers). Back: GET /api/desarrollador/red-salud (sobre leads del dev, patrón Red-de-Asesores del Dev-Master). Cierra ciclo: salud→acción→métricas. Verificado en vivo.
- ✅ **C.4 Pricing (2026-06-06, Bloque 1.4):** consolida el Pricing del dev en tabs **Estrategia · Sugerencias por Unidad** (DevPricingArea). 'Sugerencias' = la tabla AVM por unidad que ya existía (razonamiento + Aprobar/Rechazar/Aplicar). NUEVO 'Estrategia' = **Precio Inteligente** (DevPricingStrategy): recomendación por proyecto (sube/baja/promoción/sostén según ritmo de venta + vs mediana de zona del MERCADO) + elasticidad — reusa _stock_soldout(dev_ids) del Dev-Master. CTAs cierran ciclo → sugerencias por unidad → Lab de experimentos (ruta existente). Back: GET /api/desarrollador/pricing-inteligente + fix mediana de zona = mercado-wide. Verificado en vivo.
- ✅ **C.3 Inteligencia (2026-06-06, Bloque 1.3):** el Centro de Inteligencia del dev ya tenía Mercado/Demanda/Precios/Competencia/Reportes/Dónde-construir (cube/zones/CDMX/battle-card/site-selection/demand-gap). GAP vs Dev-Master = **Comportamiento**. NUEVO: área **"Qué Frena Tus Ventas"** (DevComportamiento.js, tema claro) que BAJA el lente del Dev-Master scope-ado a los leads del dev — objeciones+contra-argumento · velocidad-respuesta-vs-cierre · embudo · DISC. Back: _comportamiento(db, dev_ids=...) MISMO motor del superadmin + filtro (reuso real). GET /api/desarrollador/comportamiento. Verificado en vivo. **Bloque 1 (1.1+1.2+1.3) cerrado.** Falta Paso C: Pricing · Red Comercial · Marketing · Reportes + Fase 1-4 hardening.
- ✅ **C.2 CRM & Leads (2026-06-06, Bloque 1.2):** completa "Embudo · Leads · Ficha". El CRM ya tenía Tablero/Embudo/Bandeja + asistente IA + automatizaciones. NUEVO: ListaView → **Cockpit de Leads IA-first** (cada lead con TEMPERATURA caliente/tibio/frío + score + PRÓXIMA MEJOR ACCIÓN + resumen chips, hot-first) · clic → **Ficha** (reusa LeadDrawer exportado del board). Back: GET /api/desarrollador/leads-cockpit (heat proxy si no hay IA). **FIX cable roto:** el dev no podía abrir el lead de su propio proyecto (lead sin dev_org_id) → /api/leads/{id} ahora permite si development_id ∈ sus proyectos (scope tenant, no cross-org). Verificado en vivo.
