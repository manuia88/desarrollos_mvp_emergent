...
- **B4 MKT-3 · Despertar lo apagado ✅** — el momentum de la página de colonia leía un dato viejo; ahora muestra el motor real `live_pulse` (widget que ya existía, se auto-oculta sin dato). Y la página de barrios pasó de 16 chips de texto muerto HARDCODEADOS a una **lista DINÁMICA del catálogo real** (`/api/maps/colonias` ← colonias_catalog, que crece con el sync SIG/zonificación del módulo dev/superadmin): links reales a `/zona/:slug` que AUTO-CRECEN de 16 (seed/fallback) a todas las colonias de CDMX cuando hay datos. (Corrección tras feedback del founder: no hardcodear 16; leer el catálogo.) Cero componentes/endpoints nuevos. Build verde, 0 warnings nuevos.
- Pendiente: prender la suite agéntica (`agentic_enabled`) = decisión de deploy del founder, no código. B4 sigue (MKT-2 ficha que vende · MKT-4 a11y · MKT-5 Atlax humano · resto de portales). Decisión founder abierta: las 6 propiedades ficticias del home.
