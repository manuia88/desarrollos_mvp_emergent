...
**Preexistente (ajeno):** `tests/test_search_parser.py::test_search_parser_casos` falla también sin estos cambios (no toca motores financieros) → N4, fuera de scope. **[RESUELTO en Batch 12 ↓]**

### BATCH 12 · CIERRE DE BACKLOG A CERO DEUDA (2026-07-03 · orden del founder) — ✅ TODO CERRADO
| Item | Qué era | Cierre |
|------|---------|--------|
| A2 `demand_by_feature` | full-scan no agregable sin romper `_attribute()` | cache TTL 5 min a nivel función (cubre TODOS los callers: dev_market/superadmin/granularity) |
| A8 `substitution` | secuencias por visitante no expresables en `$group` | cache TTL 5 min a nivel función |
| B8 buyer score "visitas" | contaba citas por `user_id` que las citas NO tienen → siempre 0 | puente user→lead por `contact.email_norm` (mismo normalizado del alta) → conteo por `lead_id $in`; conserva el conteo directo por si a futuro se escribe user_id |
| D12 `v1_comparables` | bounding-box sin índice (docs sin geo poblado) | cerrado con índice compuesto `(lat,lng)` — reescritura geo cambiaría resultados |
| AUD-031 | opt-out newsletter sin token → un tercero podía desuscribir por CSRF/GET | **token HMAC-SHA256(user_id:segmento)** con el secreto del server, embebido en el link del correo; el endpoint exige `compare_digest` (403 sin token). Sin ventana de gracia: nunca se enviaron correos (no hay prod) |
| test_search_parser (preexistente) | "100m2 a 250" → `max_sqm` None. Causa raíz: el LLM parseaba "a 250" como PRECIO ($250k) y devolvía `min_sqm` parcial → el guard saltaba el completado determinista → cacheado 24h | completado determinista cuando el LLM deja el rango m² a medias (XOR) con lookahead anti-dinero (`\b(?!mil|k|millones|mdp|pesos|$)`) + descarta el precio fantasma del mismo token + bump caché v2→v3. Verificado contra el server vivo |
| AUD-025 | fragilidad de orden en 3 tests | **ya NO reproduce** (combo que fallaba: 12 passed) — el hook skip-si-no-infra del Batch 2 lo resolvió de facto. Cerrado |
| AUD-019 | `or` con operandos idénticos | corregido desde Batch 2 (la tabla decía "abierto" — corregida) |

**Suite tras el cierre: 1,258 passed · 0 failed · 472 skipped** (primera vez 100% verde). Test de regresión del cierre: `tests/test_aud_backlog_cierre.py` (9). Deuda restante de auditoría: **CERO** (los bumps de dependencias viven en DEPLOY_CHECKLIST §1.5 como paso del deploy, por diseño).
