...
// NOTA: el dato 4S (precio de equilibrio / gap radar) es EXCLUSIVO de superadmin (decisión founder
// 2026-07-12) — vive en el Hub de Mercado › Equilibrio 4S, no en el portal del desarrollador.
...
// F4.1/F4.2 · Qué Pasaría Si: factores simulables + proyección de impacto con palancas aprendidas
export const getSimuladorFactores = () => j('/api/dev/simulador-palancas/factores');
export const getSimuladorPalancas = ({ factor = 'recamaras', de, a }) =>
  j(`/api/dev/simulador-palancas?factor=${factor}&a=${encodeURIComponent(a)}${de != null ? `&de=${encodeURIComponent(de)}` : ''}`);
...
// Pulso de ventas — agotamiento vs entrega, what-if de precio, proyección, mix, margen
export const getSalesIntel = (projectId) => j(`/api/dev/projects/${projectId}/sales-intel`);
...
// Valor de mercado de la zona (suelo catastral oficial vs comercial estimado) — antes solo superadmin
export const getDevValorMercadoZona = (projectId) =>
  j(`/api/dev/valor-mercado-zona?project_id=${encodeURIComponent(projectId)}`);
