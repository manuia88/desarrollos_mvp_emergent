...
function sectionFromPath(p) {
  ...
  if (/^\/superadmin\/(scores|drpi|risk-score|investment-explorer|intelligence-hub|trends|phase5-foundation|transactions|avm-accuracy|forecast-accuracy|knowledge-graph|live-pulse|construction-quality|reviews-residents|conversations)/.test(p)) return 'inteligencia';
  ...
  if (/^\/superadmin\/(ai-cost|commercial|api-keys|vertical-products|data-licensing|cross-sell-analytics|soc-franchise|marketplace-templates|lead-enrichment|social-ads|video-standalone)/.test(p)) return 'monetizacion';
