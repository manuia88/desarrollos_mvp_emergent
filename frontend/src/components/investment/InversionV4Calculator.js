...
const SEM = { verde: '#10B981', amarillo: '#E0A33E', rojo: '#DC2626', gris: '#8A8FA6' };
...
const Auto = () => <span style={{ marginLeft: 6, fontSize: 8.5, fontWeight: 800, color: '#4F46E5', background: 'rgba(124,92,255,0.12)', borderRadius: 5, padding: '1px 5px', verticalAlign: 'middle' }}>AUTO · EDITABLE</span>;
// link directo al Proyector de Impuestos (abre en pestaña nueva para no perder la calculadora)
const ProyectorLink = () => <a href="/tools/tax-projector" target="_blank" rel="noreferrer" style={{ color: '#4F46E5', fontWeight: 700, textDecoration: 'underline' }}>Proyector de Impuestos</a>;
// globito "?" con explicación rica (qué es · de dónde sale · ejemplo real). children = contenido.
const Info = ({ children }) => <sup className="iv4-tip" tabIndex={0} style={{ marginLeft: 3 }}><span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 13, height: 13, borderRadius: '50%', background: 'rgba(124,92,255,0.14)', color: '#4F46E5', fontSize: 9, fontWeight: 800 }}>?</span><span className="iv4-tipbox" style={{ width: 250 }}>{children}</span></sup>;
  ...
  return (
    <div style={{ fontFamily: 'DM Sans', color: '#16182A' }}>
      ...
      <div ref={tusDatosRef} className="iv4-card iv4-noprint" style={{ marginBottom: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap', marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            {vista === 'institucional' && <span style={{ fontFamily: 'DM Sans', fontWeight: 800, fontSize: 12, color: '#4F46E5', background: 'rgba(124,92,255,0.1)', padding: '6px 12px', borderRadius: 9999 }}>🏛️ Institucional</span>}
            <div style={{ display: 'inline-flex', background: 'rgba(16,18,28,0.05)', borderRadius: 9999, padding: 3 }}>
              {['MXN', 'USD'].map((mo) => (
                <button key={mo} type="button" onClick={() => setMoneda(mo)} style={{ padding: '7px 13px', borderRadius: 9999, cursor: 'pointer', fontFamily: 'DM Sans', fontWeight: 800, fontSize: 12, border: 'none', background: moneda === mo ? '#fff' : 'transparent', color: moneda === mo ? '#4F46E5' : '#6B6F86', boxShadow: moneda === mo ? '0 2px 8px rgba(16,18,28,0.08)' : 'none' }}>{mo}</button>
              ))}
            </div>
          </div>
        </div>
        {moneda === 'USD' && r && r.mercado && r.mercado.fix_usd && (
          <div style={{ fontSize: 10.5, color: '#4F46E5', background: 'rgba(14,165,233,0.08)', borderRadius: 8, padding: '7px 11px', marginBottom: 12, display: 'inline-block' }}>
          ...
          <div style={{ marginBottom: 16, paddingBottom: 16, borderBottom: '1px solid rgba(16,18,28,0.08)' }}>
            ...
            {port && (
              <div style={{ marginTop: 12 }}>
                <div style={{ background: 'rgba(192,38,211,0.08)', borderRadius: 10, padding: '9px 13px', fontSize: 11.5, color: '#4F46E5', lineHeight: 1.5 }}>✓ <b>Portafolio: {port.n_unidades} unidades · {m(port.precio_total)} total.</b> Todo (cómo lo pagas, impuestos, crédito, pentágono, métricas) ya es del portafolio combinado.</div>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                    ...
                    <tbody>{port.unidades.map((u, i) => (
                      <tr key={i} style={{ borderTop: '1px solid rgba(16,18,28,0.05)' }}>
                        ...
                        <td style={{ padding: '4px 8px', textAlign: 'right', fontWeight: 700, color: (u.tir_pct || 0) >= 0 ? '#10B981' : '#DC2626' }}>{pct(u.tir_pct)}</td>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 13, flexWrap: 'wrap', alignItems: 'flex-end' }}>
              {(vista === 'institucional' && portfolioUnits.length >= 1 && port) ? (
                <div style={{ minWidth: 150 }}><span style={lab}>{port.n_unidades > 1 ? 'Precio total' : 'Precio'} <span style={{ color: '#4F46E5', fontWeight: 700 }}>· {port.n_unidades} {port.n_unidades > 1 ? 'unidades' : 'unidad'}</span></span>
                  <input type="text" readOnly value={m(port.precio_total)} style={{ ...inp, background: 'rgba(192,38,211,0.06)', color: '#4F46E5', fontWeight: 700, cursor: 'not-allowed' }} /></div>
              ) : (
        ...
        {f.modo_renta === 'corto' && zoneId && (
          <div style={{ marginTop: 12, padding: '11px 14px', background: 'rgba(14,165,233,0.08)', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, flexWrap: 'wrap' }}>
            <div style={{ fontSize: 11.5, color: '#4F46E5', lineHeight: 1.45 }}>
        ...
        <div style={{ borderTop: '1px solid rgba(16,18,28,0.07)', marginTop: 14, paddingTop: 12 }}>
          <button type="button" onClick={() => setOpenAdv((o) => !o)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', background: 'none', border: 'none', cursor: 'pointer', padding: 0, fontFamily: 'Outfit', fontWeight: 800, fontSize: 12.5, color: '#16182A' }}>
            <span>⚙️ Configuración <span style={{ fontWeight: 600, color: '#8A8FA6', fontSize: 11 }}>· predial, mantenimiento, horizonte, plusvalía, régimen fiscal</span></span><span style={{ color: '#6366F1' }}>{openAdv ? '−' : '+'}</span>
          </button>
          {openAdv && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 12, marginTop: 14 }}>
              ...
              <div><span style={lab}>N° Unidades {multifamily && <span style={{ color: '#4F46E5', fontWeight: 700 }}>·multi</span>}</span><input type="number" value={f.num_unidades} onChange={(e) => set('num_unidades', e.target.value)} style={inp} /></div>
      ...
      {vista === 'institucional' && r && (
        <div className="iv4-card" style={{ marginBottom: 12, borderLeft: '4px solid #6366F1' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(118px,1fr))', gap: 12 }}>
            {[['TIR (vende ' + f.horizonte_anios + 'a)', pct(r.tir_pct), (r.tir_pct || 0) >= 0 ? '#10B981' : '#DC2626'],
            ['Cap rate', pct(r.cap_rate_pct), '#4F46E5'],
            ...(capRateMercado != null ? [['vs mercado zona', `${(r.cap_rate_pct - capRateMercado) >= 0 ? '+' : ''}${(r.cap_rate_pct - capRateMercado).toFixed(1)} pts`, (r.cap_rate_pct - capRateMercado) >= 0 ? '#059669' : '#DC2626']] : []),
            ...((r.credito || {}).dscr != null ? [['DSCR', r.credito.dscr, (r.credito.dscr >= 1.2 ? '#10B981' : r.credito.dscr >= 1 ? '#E0A33E' : '#DC2626')]] : []),
            ...
            ['VPN', m(r.vpn), (r.vpn || 0) >= 0 ? '#10B981' : '#DC2626']].map(([l, v, c]) => (
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
                <div style={{ display: 'flex', gap: 28, flexWrap: 'wrap' }}>
                  <div>
                    ...
                    <div style={{ fontFamily: 'Outfit', fontWeight: 800, fontSize: 36, color: '#4F46E5', letterSpacing: '-0.02em', lineHeight: 1 }}>{pct(r.cap_rate_pct)}</div>
            ...
            return (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 14, alignItems: 'start' }}>
                <Grupo titulo="📈 Si es para invertir (rentarla)" sub="Lo que importa si la vas a rentar." items={[
                  ['🔑', 'Rendimiento de la renta (cap rate)', pct(r.cap_rate_pct), '#4F46E5', 'Cuánto te deja la renta sobre el precio, sin contar el crédito.', 'anual', <><b>Imagina</b> que prestas tu juguete y te dan monedas. El cap rate dice cuántas monedas te dan al año por cada 100 que vale el juguete. <b>Cómo:</b> lo que deja la renta en un año ({m(r.noi)}) ÷ precio ({m(f.valor_propiedad)}) = <b>{pct(r.cap_rate_pct)}</b>. <b>Fuente:</b> renta = promedio de la zona.</>],
                  ['💰', 'Rendimiento promedio (ROI)', pct(r.roi_anualizado_pct), '#10B981', 'Tu ganancia promedio contando TODO (renta + venta), repartida en los años.', 'anual', <>Junta TODO lo que ganas (la renta + lo que sube de valor al vender) y lo reparte entre los años que lo tienes. <b>Tu caso:</b> ~<b>{pct(r.roi_anualizado_pct)}</b> al año. Es como sacar el promedio de tus calificaciones de todo el año.</>],
                  ['🏦', 'Flujo de la renta', m(r.flujo_mensual_1) + '/mes', (r.flujo_mensual_1 || 0) >= 0 ? '#10B981' : '#DC2626', 'Lo que te queda (o sale de tu bolsa) cada mes tras gastos y crédito.', 'mensual', <>Es tu domingo cada mes: lo que entra de renta menos lo que sale (gastos + mensualidad del banco). <b>Tu caso:</b> <b>{m(r.flujo_mensual_1)}/mes</b>. Si es negativo (rojo), tú pones esa diferencia.</>],
                  ['✖️', 'Multiplicas tu dinero', r.equity_multiple ? `${r.equity_multiple}x` : '—', '#6366F1', 'Por cada peso que pones, cuántos recuperas al final.', 'total', <>Por cada <b>$1</b> que pones de tu bolsa, cuántos recuperas al final. <b>Tu caso: {r.equity_multiple}x</b> → metes $1 y al final te llevas ${r.equity_multiple}. Más de 1 = ganas; menos de 1 = pierdes.</>],
            ...
            <div className="iv4-card">
              ...
              {r.desglose.escrituracion_detalle && (
                <details style={{ marginTop: 8 }}>
                  <summary style={{ cursor: 'pointer', fontSize: 11, fontWeight: 800, color: '#6366F1' }}>¿De qué se compone la escrituración?</summary>
              ...
              {r.con_credito && r.credito && <div style={{ display: 'flex', justifyContent: 'space-between', padding: '9px 0 0', marginTop: 6, borderTop: '1px solid rgba(16,18,28,0.05)', fontSize: 12 }}><span style={{ color: '#6B6F86' }}>De tu bolsa hoy (enganche + gastos)</span><span style={{ fontWeight: 800, color: '#6366F1' }}>{m(r.credito.capital_propio)}</span></div>}
            </div>
          )}
          ...
          {r && r.venta && (
            <div className="iv4-card">
              ...
              ['= Te llevas (neto)', r.venta.neto, '#10B981', true, <>Lo que de verdad te queda en la bolsa: precio de venta − comisión − ISR{r.con_credito ? ' − lo que debes al banco' : ''}. <b>Tu caso:</b> {m(r.venta.neto)}.</>]].map(([l, v, c, tot, info]) => (
            ...
            return (
              <div className="iv4-card" style={{ gridColumn: '1 / -1' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'stretch', gap: 8, flexWrap: 'wrap' }}>
                  {[['Precio del depa', precio, '#16182A'], ['=', null], ['Tu enganche', engPuro, '#6366F1'], ['+', null], ['Te prestan', cr.monto_credito, '#10B981']].map(([l, v, c], i) => (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(120px,1fr))', gap: 12 }}>
                  ...
                  <Tile l="La renta cubre" v={pct(cr.cobertura_renta_pct)} c={(cr.cobertura_renta_pct || 0) >= 100 ? '#10B981' : '#DC2626'} exp="Cuánto de la mensualidad paga la renta." />
                </div>
                <div style={{ marginTop: 12, padding: '11px 13px', background: 'rgba(16,18,28,0.03)', borderRadius: 10 }}>
                  ...
                  <div style={{ display: 'flex', height: 12, borderRadius: 6, overflow: 'hidden', marginBottom: 7 }}>
                    <div style={{ width: `${capPct}%`, background: '#6366F1' }} /><div style={{ width: `${100 - capPct}%`, background: '#DC2626' }} />
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, fontSize: 11, fontWeight: 700, flexWrap: 'wrap' }}>
                    <span style={{ color: '#6366F1' }}>🟪 {m(cr.capital_anio1)} a capital ({capPct}%)</span>
                ...
                {cr.tabla_anual && cr.tabla_anual.length > 0 && (
                  <details style={{ marginTop: 12, border: '1px solid rgba(124,92,255,0.25)', borderRadius: 10, background: 'rgba(124,92,255,0.04)', padding: '10px 13px' }}>
                    <summary style={{ cursor: 'pointer', fontSize: 12, fontWeight: 800, color: '#4F46E5' }}>📅 ¿Y el 2°, 3°… año? Velo año por año hasta liquidar</summary>
                    <div style={{ overflowX: 'auto', marginTop: 10 }}>
                      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                        ...
                        <tbody>{cr.tabla_anual.map((row) => { const tot = row.capital + row.interes; const capPc = tot ? Math.round(row.capital / tot * 100) : 0; return (
                          <tr key={row.anio} style={{ borderTop: '1px solid rgba(16,18,28,0.05)' }}>
                            ...
                            <td style={{ padding: '5px 8px', textAlign: 'right', color: '#6366F1', fontWeight: 700 }}>{m(row.capital)} <span style={{ color: '#A2A6BC', fontWeight: 500 }}>({capPc}%)</span></td>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(120px,1fr))', gap: 12 }}>
                  <Tile l="Capital (préstamo)" v={m(cr.monto_credito)} c="#6366F1" exp="El préstamo que le regresas al banco." />
                ...
                {cr.abono && (
                  <div style={{ marginTop: 14, padding: '10px 12px', background: 'rgba(14,159,110,0.08)', borderRadius: 10, fontSize: 11.5, color: '#059669', lineHeight: 1.5 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginTop: 14 }}>
          ...
          {r && r.comparar_renta && r.comparar_renta.largo && (() => {
            ...
            const Opcion = ({ icon, titulo, x, comoIngreso, fuente, win, headInfo, gastosInfo }) => (
              <div style={{ flex: '1 1 240px', padding: '15px 16px', borderRadius: 14, background: win ? 'rgba(124,92,255,0.06)' : 'rgba(16,18,28,0.03)', border: win ? '1.5px solid #6366F1' : '1px solid rgba(16,18,28,0.06)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
                  <div style={{ fontSize: 13, fontWeight: 800, color: '#16182A' }}>{icon} {titulo}<Info>{headInfo}</Info></div>
                  {win && <span style={{ fontSize: 9.5, fontWeight: 800, color: '#4F46E5', background: 'rgba(124,92,255,0.14)', borderRadius: 6, padding: '2px 8px' }}>GANA</span>}
                </div>
                <div style={{ marginTop: 11, fontSize: 11.5, color: '#5B5F76', lineHeight: 1.5 }}>
                  ...
                  <div style={{ marginTop: 5, paddingTop: 6, borderTop: '1px dashed rgba(16,18,28,0.14)' }}>= Te queda: <b style={{ color: '#10B981' }}>{m(x.noi)}/año</b><Info><>El <b>NOI</b>: ingreso − gastos. Lo que deja la propiedad antes del crédito y de impuestos. <b>Tu caso:</b> {m(x.ingreso_anual)} − {m(x.egresos_anual)} = {m(x.noi)}/año.</></Info></div>
                </div>
                <div style={{ marginTop: 11, display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                  <div><div style={{ fontSize: 9.5, color: '#6B6F86', fontWeight: 700 }}>Renta al año (cap rate)<Info><>Lo que deja la renta sobre el precio cada año, sin contar venta ni crédito. Estable. <b>Tu caso:</b> {pct(x.cap_rate_pct)}.</></Info></div><div style={{ fontFamily: 'Outfit', fontWeight: 800, fontSize: 16, color: '#4F46E5' }}>{pct(x.cap_rate_pct)}</div></div>
                  <div><div style={{ fontSize: 9.5, color: '#6B6F86', fontWeight: 700 }}>Si vendes (TIR)<Info><>Rendimiento anual juntando renta + plusvalía si vendes al año {f.horizonte_anios}. <b>Tu caso:</b> {pct(x.tir_pct)}.</></Info></div><div style={{ fontFamily: 'Outfit', fontWeight: 800, fontSize: 18, color: (x.tir_pct || 0) >= 0 ? '#10B981' : '#DC2626' }}>{pct(x.tir_pct)}</div></div>
            ...
            return (
              <div className="iv4-card" style={{ gridColumn: '1 / -1' }}>
                ...
                <div style={{ marginTop: 12, padding: '11px 13px', background: 'rgba(124,92,255,0.06)', borderRadius: 10, fontSize: 11.5, color: '#5B5F76', lineHeight: 1.55 }}>
                  👉 Con tus datos <b style={{ color: '#4F46E5' }}>gana {cmp.gana === 'corto' ? 'Airbnb' : 'largo plazo'}</b> (deja más al año, <b>ya descontados</b> sus mayores gastos). <b>Airbnb</b> rinde más pero da más trabajo; <b>largo plazo</b> rinde menos pero es estable y sin broncas. Toca cada <b>?</b> para ver de dónde sale el número.
                </div>
              </div>
            );
          })()}
          ...
          {r && r.atribucion && (() => {
            ...
            const segs = [['Renta', a.renta_neta_acum, '#10B981'], ['Patrimonio', a.equity_buildup, '#6366F1'], ['Plusvalía', a.plusvalia, '#4F46E5']].filter(([, v]) => (v || 0) > 0);
            return (
              <div className="iv4-card">
              ...
              <div className="iv4-card">
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11.5 }}>
                    ...
                    <tbody>{filas.map((x, fi) => (
                      <tr key={fi} style={{ borderTop: '1px solid rgba(16,18,28,0.06)', background: x.hero ? 'rgba(124,92,255,0.07)' : 'transparent' }}>
                        <td style={{ padding: '8px', fontWeight: 800, color: x.hero ? '#4F46E5' : '#16182A', position: 'sticky', left: 0, background: x.hero ? '#EEF2FF' : '#fff', whiteSpace: 'nowrap' }}>{x.hero ? '🏠 ' : ''}{x.nombre}{x.ejemplos && !x.hero ? <span style={{ display: 'block', fontSize: 9, fontWeight: 600, color: '#A2A6BC' }}>ej. {x.ejemplos}</span> : null}</td>
                        {cols.map(([l, get], ci) => <td key={l} style={{ padding: '8px', whiteSpace: 'nowrap', color: ci === 0 ? (x.hero ? '#4F46E5' : '#16182A') : '#5B5F76', fontWeight: ci === 0 ? 800 : 600 }}>{get(x)}</td>)}
                      </tr>
                    ))}</tbody>
                  </table>
                </div>
                ...
                <div style={{ marginTop: 12, padding: '12px 14px', background: 'rgba(16,18,28,0.025)', borderRadius: 10 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 8 }}>
                    {cols.map(([l, , leg]) => <div key={l} style={{ fontSize: 10.5, color: '#5B5F76', lineHeight: 1.5 }}><b style={{ color: '#4F46E5' }}>{l}:</b> {leg}</div>)}
                  </div>
                </div>
              ...
              <div className="iv4-card">
                ...
                <div style={{ display: 'flex', gap: 18, flexWrap: 'wrap', alignItems: 'center', marginTop: 8 }}>
                  <svg viewBox="-34 0 368 290" style={{ width: 310, maxWidth: '100%' }}>
                    ...
                    <polygon points={polyOf(sI)} fill="rgba(124,92,255,0.20)" stroke="#6366F1" strokeWidth="2.5" />
                  ...
                  <div style={{ fontSize: 11.5, lineHeight: 1.6, flex: '1 1 180px' }}>
                    <div><span style={{ display: 'inline-block', width: 11, height: 11, borderRadius: 3, background: '#6366F1', marginRight: 6 }} /><b>Tu inmueble</b> (TIR {pct(r.tir_pct)})</div>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6, height: 56, marginBottom: 12 }}>
                {r.proyeccion.rows.map((row) => { const mx = Math.max(...r.proyeccion.rows.map((x) => x.valor || 0)) || 1; const best = row.anio === r.proyeccion.mejor_anio; return (
                  <div key={row.anio} style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', alignItems: 'center' }}>
                    <div style={{ width: '70%', height: `${Math.max(6, (row.valor / mx) * 44)}px`, background: best ? 'linear-gradient(180deg,#6366F1,#4F46E5)' : '#C7CAD6', borderRadius: '4px 4px 0 0' }} title={m(row.valor)} />
                    <div style={{ fontSize: 9, color: best ? '#4F46E5' : '#8A8FA6', fontWeight: best ? 800 : 600, marginTop: 4 }}>{row.anio}a</div>
                  </div>
                ); })}
              </div>
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11.5 }}>
                  ...
                  {r.proyeccion.rows.map((row) => { const best = row.anio === r.proyeccion.mejor_anio; const dif = row.diferencial_mensual || 0; return (
                    <tr key={row.anio} style={{ borderTop: '1px solid rgba(16,18,28,0.06)', background: best ? 'rgba(124,92,255,0.07)' : 'transparent' }}>
                      <td style={{ padding: '7px 5px', fontWeight: 800, color: best ? '#4F46E5' : '#16182A' }}>{row.anio}{best ? ' ⭐' : ''}</td>
                      <td style={{ padding: '7px 5px', textAlign: 'right' }}>{m(row.valor)}</td>
                      <td style={{ padding: '7px 5px', textAlign: 'right', color: '#10B981' }}>+{m(row.plusvalia_acum)}</td>
                      ...
                      <td style={{ padding: '7px 5px', textAlign: 'right', fontWeight: 700, color: dif >= 0 ? '#10B981' : '#DC2626' }}>{m(dif)}/mes</td>
                      <td style={{ padding: '7px 5px', textAlign: 'right', fontWeight: 800, color: (row.tir_si_vendes || 0) >= 0 ? '#6366F1' : '#DC2626' }}>{pct(row.tir_si_vendes)}</td>
                    </tr>
                  ); })}</tbody>
                </table>
              </div>
              <div style={{ fontSize: 9.5, color: '#A2A6BC', fontStyle: 'italic', marginTop: 8 }}>⭐ = el mejor año para salir (TIR máxima). <b style={{ color: '#DC2626' }}>Diferencial en rojo</b> = pones de tu bolsa al mes (la renta aún no cubre el crédito); <b style={{ color: '#10B981' }}>en verde</b> = te sobra. La mensualidad es fija y la renta sube cada año, así que con el tiempo mejora.</div>
            </div>
          )}
      ...
      {vista === 'institucional' && r && (() => {
        ...
        const mets = [
          ['Cap rate', pct(r.cap_rate_pct), '#4F46E5', <>Renta neta ÷ precio. El rendimiento <b>anual</b> de la renta, sin contar el crédito. <b>Tu caso:</b> {pct(r.cap_rate_pct)}.</>],
          ['TIR (con crédito)', pct(r.tir_pct), '#4F46E5', <>Rendimiento anual total <b>apalancado</b> (renta + plusvalía) si vendes al año {f.horizonte_anios}. <b>Tu caso:</b> {pct(r.tir_pct)}.</>],
          ...
          ['VPN', m(r.vpn), (r.vpn || 0) >= 0 ? '#10B981' : '#DC2626', <><b>Valor Presente Neto:</b> cuánto ganas (o pierdes) <b>hoy</b>, en pesos de hoy, por encima de tu tasa de oportunidad ({pct(r.tasa_descuento_pct)}). Positivo = crea valor. <b>Tu caso:</b> {m(r.vpn)}.</>],
          ...
          ['Multiplicas', r.equity_multiple ? `${r.equity_multiple}x` : '—', '#6366F1', <>Por cada <b>$1</b> que pones de tu bolsa, cuántos recuperas al final (renta + venta). <b>Tu caso:</b> {r.equity_multiple}x.</>],
          ...(cr.dscr != null ? [['DSCR', cr.dscr, (cr.dscr >= 1.2 ? '#10B981' : cr.dscr >= 1 ? '#E0A33E' : '#DC2626'), <><b>Cobertura del crédito</b> (Debt Service Coverage Ratio): NOI ÷ pago anual al banco. Mayor a 1 = la renta cubre el crédito; los bancos suelen pedir <b>≥1.2</b>. <b>Tu caso:</b> {cr.dscr}.</>]] : []),
          ...
          ...((r.proyeccion || {}).payback_anio ? [['Recuperas tu inversión', `año ${r.proyeccion.payback_anio}`, '#10B981', <>El año en que lo que te llevas al vender ya <b>recupera lo que pusiste</b> de tu bolsa. Antes de eso, todavía no "sales tablas". <b>Tu caso:</b> ~año {r.proyeccion.payback_anio}.</>]] : []),
        ];
        return (
          <div className="iv4-card" style={{ marginTop: 12, borderTop: '4px solid #6366F1' }}>
                ...
                <div style={{ marginTop: 12, paddingTop: 10, borderTop: '1px solid rgba(16,18,28,0.07)' }}>
                  ...
                  {d && <div style={{ marginBottom: 10 }}>
                    ...
                    <div style={{ display: 'flex', height: 14, borderRadius: 6, overflow: 'hidden', marginBottom: 5 }}>
                      {[['#10B981', Math.max(0, d.renta)], ['#6366F1', Math.max(0, d.patrimonio)], ['#4F46E5', Math.max(0, d.plusvalia)]].map(([c, v], i) => <div key={i} style={{ width: `${Math.min(100, v)}%`, background: c }} />)}
                    </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(130px,1fr))', gap: 12 }}>
                    {[['Spread vs CETES', `${ai.spread_vs_cetes_pts >= 0 ? '+' : ''}${ai.spread_vs_cetes_pts} pts`, ai.spread_vs_cetes_pts >= 0 ? '#10B981' : '#DC2626', <>Cap rate − CETES = la <b>prima de riesgo</b> que te paga el inmueble sobre lo seguro. Positivo = el precio compensa el riesgo; negativo = pagas caro vs CETES.</>],
                    ...
                    ['Préstamo máx (DSCR 1.2)', m(ai.prestamo_max_dscr12), sobreApal ? '#DC2626' : '#10B981', <>Lo máximo que un banco te prestaría para que la renta cubra el crédito a <b>1.2x</b> (estándar). Si tu crédito lo supera, estás sobre-apalancado.</>]].map(([l, v, c, info]) => (
                  ...
                  {ai.metricas_fondo && (() => { const mfo = ai.metricas_fondo; return (
                    <div style={{ marginTop: 12, paddingTop: 10, borderTop: '1px dashed rgba(16,18,28,0.1)' }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(92px,1fr))', gap: 10 }}>
                        {[['TWR (s/ apal.)', pct(mfo.twr_unlev_pct), '#059669'], ...(mfo.si_irr_pct != null ? [['SI-IRR', pct(mfo.si_irr_pct), (mfo.si_irr_pct || 0) >= 0 ? '#10B981' : '#DC2626']] : []), ['TVPI', `${mfo.tvpi}×`, '#6366F1'], ['DPI', `${mfo.dpi}×`, '#16182A'], ['RVPI', `${mfo.rvpi}×`, '#4F46E5'], ['PIC', m(mfo.pic), '#16182A'], ...(mfo.tger_pct != null ? [['TGER', pct(mfo.tger_pct), '#6B6F86']] : [])].map(([l, v, c]) => (
              ...
              return (
                <div style={{ marginTop: 12, paddingTop: 10, borderTop: '1px solid rgba(16,18,28,0.07)' }}>
                  ...
                  <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', alignItems: 'center' }}>
                    <div><div style={{ fontSize: 10, color: '#6B6F86', fontWeight: 700 }}>Tu cap rate</div><div style={{ fontFamily: 'Outfit', fontWeight: 800, fontSize: 18, color: '#4F46E5' }}>{pct(r.cap_rate_pct)}</div></div>
                    ...
                    <div style={{ padding: '8px 12px', borderRadius: 10, background: dif >= 0 ? 'rgba(14,159,110,0.1)' : 'rgba(220,38,38,0.08)', color: dif >= 0 ? '#059669' : '#DC2626', fontWeight: 800, fontSize: 11.5 }}>{dif >= 0 ? `✓ Entras mejor: +${dif} pts de yield` : `⚠️ Entras caro: ${dif} pts vs el mercado`}</div>
                  </div>
                </div>
              );
            })()}
              ...
              <div style={{ marginTop: 12, paddingTop: 10, borderTop: '1px solid rgba(16,18,28,0.07)' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))', gap: 10 }}>
                  {[['🟢 Optimista', 'optimista'], ['⚪ Base (tus datos)', 'base'], ['🔴 Pesimista', 'pesimista']].map(([titulo, k]) => { const s = r.escenarios[k] || {}; return (
                    <div key={k} style={{ padding: '11px 13px', borderRadius: 10, background: k === 'base' ? 'rgba(124,92,255,0.07)' : 'rgba(16,18,28,0.03)', border: k === 'base' ? '1.5px solid #6366F1' : '1px solid transparent' }}>
                      <div style={{ fontWeight: 800, fontSize: 12, color: '#16182A' }}>{titulo}</div>
                      <div style={{ fontFamily: 'Outfit', fontWeight: 800, fontSize: 18, color: (s.tir_pct || 0) >= 0 ? '#10B981' : '#DC2626', marginTop: 4 }}>{pct(s.tir_pct)} <span style={{ fontSize: 10, color: '#8A8FA6', fontWeight: 600 }}>TIR</span></div>
            ...
            {r.sensibilidad && r.sensibilidad.por_exit_cap && (
              <div style={{ marginTop: 12, paddingTop: 10, borderTop: '1px solid rgba(16,18,28,0.07)' }}>
                ...
                <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>{r.sensibilidad.por_exit_cap.map((s) => <div key={s.exit_cap_pct} style={{ textAlign: 'center', padding: '6px 9px', borderRadius: 8, background: s.es_base ? 'rgba(124,92,255,0.1)' : 'rgba(16,18,28,0.04)', border: s.es_base ? '1.5px solid #6366F1' : '1px solid transparent' }}><div style={{ fontSize: 9.5, color: '#8A8FA6' }}>{s.exit_cap_pct}%</div><div style={{ fontFamily: 'Outfit', fontWeight: 800, fontSize: 13, color: s.es_base ? '#4F46E5' : '#16182A' }}>{pct(s.tir_pct)}</div></div>)}</div>
              </div>
            )}
                  ...
                  <table style={{ borderCollapse: 'collapse', fontSize: 10.5 }}>
                    ...
                    <tbody>{r.sensibilidad.por_tasa_aprec.filas.map((f) => (
                      <tr key={f.tasa_pct}>
                        ...
                        {f.celdas.map((c, ci) => { const t = c.tir_pct || 0; const bg = t < 0 ? '#FCE4E4' : t < 7 ? '#FCF1DD' : '#D1FAE5'; const col = t < 0 ? '#DC2626' : t < 7 ? '#8A6A1E' : '#059669'; return <td key={ci} style={{ padding: '6px 9px', textAlign: 'center', fontWeight: 800, color: col, background: bg, border: c.es_base ? '2px solid #6366F1' : '1px solid #fff' }}>{pct(c.tir_pct)}</td>; })}
                      </tr>
                    ))}</tbody>
                  </table>
                </div>
            ...
            {r.montecarlo && (
              <div style={{ marginTop: 12, paddingTop: 10, borderTop: '1px solid rgba(16,18,28,0.07)' }}>
                ...
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 8 }}>{[['Peor (5%)', r.montecarlo.p5, '#DC2626'], ['Esperado', r.montecarlo.p50, '#16182A'], ['Mejor (95%)', r.montecarlo.p95, '#10B981']].map(([l, v, c]) => <div key={l} style={{ flex: '1 1 80px', textAlign: 'center', padding: '8px', borderRadius: 8, background: 'rgba(16,18,28,0.03)' }}><div style={{ fontSize: 10, color: '#8A8FA6' }}>{l}</div><div style={{ fontFamily: 'Outfit', fontWeight: 800, fontSize: 15, color: c }}>{pct(v)}</div></div>)}</div>
            ...
            {r.proforma && r.proforma.length > 0 && (
              <div style={{ marginTop: 12, paddingTop: 10, borderTop: '1px solid rgba(16,18,28,0.07)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 6 }}>
                  ...
                  <button type="button" onClick={() => { const data = [['Año', 'Ingreso bruto', 'NOI', 'Servicio deuda', 'Flujo libre'], ...r.proforma.map((p) => [p.anio, p.ingreso_bruto, p.noi, p.servicio_deuda, p.flujo_libre])]; const csv = data.map((row) => row.join(',')).join('\n'); const a = document.createElement('a'); a.href = 'data:text/csv;charset=utf-8,' + encodeURIComponent(csv); a.download = 'proforma-inversion.csv'; a.click(); }} style={{ padding: '6px 11px', borderRadius: 8, border: '1px solid rgba(99,102,241,0.25)', cursor: 'pointer', fontFamily: 'DM Sans', fontWeight: 700, fontSize: 11, background: '#fff', color: '#6366F1' }}>⬇️ CSV</button>
                </div>
                <div style={{ overflowX: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                    ...
                    <tbody>{r.proforma.map((p) => (
                      <tr key={p.anio} style={{ borderTop: '1px solid rgba(16,18,28,0.05)' }}>
                        ...
                        <td style={{ padding: '5px 7px', textAlign: 'right', fontWeight: 800, color: (p.flujo_libre || 0) >= 0 ? '#10B981' : '#DC2626' }}>{m(p.flujo_libre)}</td>
                      </tr>
                    ))}</tbody>
                  </table>
                </div>
      ...
      {r && (
        <div className="iv4-noprint" style={{ marginTop: 16, padding: '18px 22px', borderRadius: 16, background: 'linear-gradient(120deg, #6366F1, #4F46E5)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
      ...
      {showSticky && r && createPortal(
        <div className="iv4-noprint" style={{ position: 'fixed', left: 0, right: 0, bottom: 0, zIndex: 1200, background: 'rgba(255,255,255,0.97)', backdropFilter: 'blur(6px)', borderTop: '1px solid rgba(16,18,28,0.1)', boxShadow: '0 -6px 22px rgba(16,18,28,0.1)', padding: '9px 16px' }}>
          <div style={{ maxWidth: 1140, margin: '0 auto', display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
            <div><div style={{ fontSize: 9, color: '#6B6F86', fontWeight: 700 }}>Renta/año</div><div style={{ fontFamily: 'Outfit', fontWeight: 800, fontSize: 16, color: '#4F46E5', lineHeight: 1 }}>{pct(r.cap_rate_pct)}</div></div>
            ...
            <button type="button" onClick={() => tusDatosRef.current && tusDatosRef.current.scrollIntoView({ behavior: 'smooth', block: 'start' })} style={{ marginLeft: 'auto', padding: '6px 12px', borderRadius: 9, border: '1px solid rgba(99,102,241,0.25)', cursor: 'pointer', fontFamily: 'DM Sans', fontWeight: 700, fontSize: 11.5, background: '#fff', color: '#6366F1' }}>↑ Editar todo</button>
          </div>
        </div>, document.body)}
    </div>
  );
