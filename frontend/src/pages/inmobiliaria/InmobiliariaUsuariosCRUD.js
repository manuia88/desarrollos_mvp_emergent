// Phase 14 · Batch 37 — InmobiliariaUsuariosCRUD
// Equipo interno de inmobiliaria con magic link invitation
...
const INM_ROLES = [
  ...
  { value: 'inmobiliaria_advisor', label: 'Asesor' },
  { value: 'inmobiliaria_marketing', label: 'Marketing' },
];
const ROLE_LABEL = Object.fromEntries(INM_ROLES.map(r => [r.value, r.label]));
ROLE_LABEL['inmobiliaria_admin'] = 'Administrador';
const ROLE_COLOR = {
  inmobiliaria_admin: '#EC4899', inmobiliaria_director: '#6366F1',
  inmobiliaria_advisor: '#22c55e', inmobiliaria_marketing: '#a855f7',
};

const STATUS_CFG = {
  active: { label: 'Activo', color: '#4ADE80', bg: 'rgba(74,222,128,0.10)', bd: 'rgba(74,222,128,0.28)' },
  pending_invitation: { label: 'Invitación pendiente', color: '#FACC15', bg: 'rgba(250,204,21,0.10)', bd: 'rgba(250,204,21,0.28)' },
  pending: { label: 'Invitación pendiente', color: '#FACC15', bg: 'rgba(250,204,21,0.10)', bd: 'rgba(250,204,21,0.28)' },
...
function InviteModal({ onClose, onSuccess }) {
  ...
  const submit = async () => {
    ...
    setBusy(true); setErr('');
    try { onSuccess(await inviteInmUser({ email: form.email.trim(), role: form.role, name: form.name || undefined })); }
    ...
    <div onClick={e => { if (e.target === e.currentTarget) onClose(); }} style={{ position: 'fixed', inset: 0, background: 'rgba(6,8,15,0.82)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: Z.MODAL, padding: 16 }}>
      <div style={{ background: 'rgba(13,17,28,0.97)', border: '1px solid rgba(255,255,255,0.10)', borderRadius: 18, width: '100%', maxWidth: 440, padding: '26px 26px 22px' }}>
        ...
        <div style={{ marginBottom: 13 }}>
          ...
          <input data-testid="inm-invite-email" style={inp} type="email" placeholder="usuario@empresa.com" value={form.email} onChange={e => s('email', e.target.value)} />
        </div>
        <div style={{ marginBottom: 13 }}>
          <label style={lbl}>Rol *</label>
          <div style={{ position: 'relative' }}>
            <select data-testid="inm-invite-role" style={{ ...inp, appearance: 'none', paddingRight: 36, cursor: 'pointer' }} value={form.role} onChange={e => s('role', e.target.value)}>
              <option value="">Seleccionar rol</option>
              {INM_ROLES.map(r => <option key={r.value} value={r.value}>{r.label}</option>)}
            </select>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <input data-testid="inm-invite-name" style={inp} placeholder="Nombre completo" value={form.name} onChange={e => s('name', e.target.value)} maxLength={100} />
        </div>
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          ...
          <button data-testid="inm-invite-submit" onClick={submit} disabled={busy} style={{ padding: '9px 20px', borderRadius: 9999, background: 'linear-gradient(90deg,#6366F1,#EC4899)', border: 'none', color: '#fff', fontFamily: 'DM Sans', fontWeight: 700, fontSize: 13, cursor: busy ? 'wait' : 'pointer', opacity: busy ? 0.7 : 1 }}>
...
export default function InmobiliariaUsuariosCRUD({ user, onLogout }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { const r = await getInmInternalUsers(); setItems(r.items || []); }
    catch { setItems([]); }
  ...
  const handleSuspend = async (email) => {
    ...
    try { await suspendInmUser(email); setToast('Usuario suspendido'); load(); }
    catch (e) { setToast(e.message || 'Error'); }
    finally { setBusy(b => ({ ...b, [email]: false })); }
  };

  const handleResend = async (invId) => {
    try { await resendDevInvitation(invId); setToast('Invitación reenviada'); }
    catch (e) { setToast(e.message || 'Error'); }
  };
  ...
  return (
    <InmobiliariaLayout user={user} onLogout={onLogout}>
      <div data-testid="inmobiliaria-usuarios" style={{ maxWidth: 880 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 22, flexWrap: 'wrap', gap: 12 }}>
          <div>
            ...
            <p style={{ fontFamily: 'DM Sans', fontSize: 13, color: 'rgba(240,235,224,0.50)', margin: 0 }}>Usuarios internos de la inmobiliaria.</p>
          </div>
          <button data-testid="inm-invite-user-btn" onClick={() => setShowInvite(true)} style={{ padding: '10px 20px', borderRadius: 9999, background: 'linear-gradient(90deg,#6366F1,#EC4899)', border: 'none', color: '#fff', fontFamily: 'DM Sans', fontWeight: 700, fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 7 }}>
        ...
        {loading ? (
          ...
          <div data-testid="inm-usuarios-empty" style={{ textAlign: 'center', padding: 60, color: 'rgba(240,235,224,0.40)', fontFamily: 'DM Sans' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {items.map(u => (
              <div key={u.email} data-testid={`inm-user-row-${u.email}`}
                ...
                <div style={{ display: 'flex', gap: 7, alignItems: 'center', flexWrap: 'wrap' }}>
                  ...
                  {(u.assigned_dev_partnerships || []).length > 0 && (
                    <span style={{ fontFamily: 'DM Sans', fontSize: 11, color: 'rgba(240,235,224,0.38)' }}>{u.assigned_dev_partnerships.length} alianzas</span>
                  )}
                </div>
                <div style={{ display: 'flex', gap: 6 }}>
                  {(u.status === 'pending_invitation' || u.status === 'pending') && u.invitation_id && (
                    <button data-testid={`inm-resend-btn-${u.email}`} onClick={() => handleResend(u.invitation_id)}
                  ...
                  {u.status === 'active' && (
                    <button data-testid={`inm-suspend-btn-${u.email}`} onClick={() => handleSuspend(u.email)} disabled={busy[u.email]}
      ...
      {showInvite && <InviteModal onClose={() => setShowInvite(false)} onSuccess={() => { setShowInvite(false); setToast('Invitación enviada'); load(); }} />}
    </InmobiliariaLayout>
  );
}
