// Phase 14 · Batch 37 — InmobiliariaMiniMarket
// Inventario: dev_partnerships propias + cross-inmobiliaria si allow_external_inventory
...
const SOURCE_CFG = {
  dev_partnership: { label: 'Alianza directa', color: '#818CF8', bg: 'rgba(99,102,241,0.10)', bd: 'rgba(99,102,241,0.28)' },
  cross_inmobiliaria: { label: 'Cross-inmobiliaria', color: '#EC4899', bg: 'rgba(236,72,153,0.10)', bd: 'rgba(236,72,153,0.28)' },
};

function SourceBadge({ source }) {
  const cfg = SOURCE_CFG[source] || SOURCE_CFG.dev_partnership;
  return (
...
function ProjectCard({ project }) {
  ...
  return (
    <div data-testid={`inm-mm-project-${project.id}`}
      style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, overflow: 'hidden', transition: 'border-color 220ms, transform 220ms' }}
...
export default function InmobiliariaMiniMarket({ user, onLogout }) {
  ...
  const isAdmin = user?.role === 'inmobiliaria_admin' || user?.role === 'superadmin';

  const load = useCallback(async () => {
    setLoading(true);
    try {
      const r = await getInmMiniMarket();
      setItems(r.items || []);
      setExternalEnabled((r.items || []).some(p => p.source === 'cross_inmobiliaria'));
    } catch { setItems([]); }
  ...
  const toggleExternal = async () => {
    ...
    try {
      ...
      await setInmExternalInventory(newVal);
      setExternalEnabled(newVal);
      setToast(newVal ? 'Inventario externo habilitado' : 'Inventario externo deshabilitado');
      load();
    } catch (e) { setToast(e.message || 'Error al actualizar'); }
    finally { setBusyToggle(false); }
  };

  const filtered = filterSource ? items.filter(p => p.source === filterSource) : items;
  const directCount = items.filter(p => p.source === 'dev_partnership').length;
  const crossCount = items.filter(p => p.source === 'cross_inmobiliaria').length;
      ...
      <div data-testid="inmobiliaria-mini-market" style={{ maxWidth: 1200 }}>
        ...
        <div style={{ marginBottom: 22 }}>
          ...
          <p style={{ fontFamily: 'DM Sans', fontSize: 13, color: 'rgba(240,235,224,0.50)', margin: 0 }}>
            Inventario disponible: alianzas directas con desarrolladores y cross-inmobiliaria.
          </p>
        </div>
        ...
        <div style={{ display: 'flex', gap: 14, marginBottom: 18, flexWrap: 'wrap' }}>
          <div style={{ flex: '1 1 200px', padding: '14px 18px', borderRadius: 12, background: 'rgba(99,102,241,0.08)', border: '1px solid rgba(99,102,241,0.22)' }}>
            <div style={{ fontFamily: 'DM Sans', fontSize: 11, color: 'rgba(240,235,224,0.50)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 4 }}>Alianzas directas</div>
            <div style={{ fontFamily: 'Outfit', fontWeight: 800, fontSize: 24, color: '#818CF8' }}>{directCount}</div>
          </div>
          <div style={{ flex: '1 1 200px', padding: '14px 18px', borderRadius: 12, background: 'rgba(236,72,153,0.08)', border: '1px solid rgba(236,72,153,0.22)' }}>
            <div style={{ fontFamily: 'DM Sans', fontSize: 11, color: 'rgba(240,235,224,0.50)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 4 }}>Cross-inmobiliaria</div>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 18, flexWrap: 'wrap', alignItems: 'center' }}>
          {isAdmin && (
            <button
              data-testid="inm-toggle-external-btn"
            ...
            >
              ...
              Inventario cruzado: {externalEnabled ? 'Activo' : 'Inactivo'}
            </button>
          )}
          <div style={{ display: 'flex', gap: 6 }}>
            {[['', 'Todos'], ['dev_partnership', 'Directas'], ['cross_inmobiliaria', 'Cruzadas']].map(([k, l]) => (
              <button key={k || 'all'} data-testid={`inm-filter-mm-${k || 'all'}`}
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: 70, color: 'rgba(240,235,224,0.40)', fontFamily: 'DM Sans', fontSize: 13 }}>Cargando inventario…</div>
        ) : filtered.length === 0 ? (
          <div data-testid="inm-mm-empty" style={{ textAlign: 'center', padding: 70, color: 'rgba(240,235,224,0.40)', fontFamily: 'DM Sans' }}>
            <Layers size={38} color="rgba(240,235,224,0.20)" style={{ marginBottom: 12 }} />
            <div style={{ fontFamily: 'Outfit', fontWeight: 700, fontSize: 17, color: 'var(--cream)', marginBottom: 5 }}>Sin proyectos disponibles</div>
            <div>{isAdmin ? 'Activa el inventario externo o crea alianzas con desarrolladores.' : 'No hay proyectos asignados aún.'}</div>
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 14 }}>
            {filtered.map(p => <ProjectCard key={`${p.id}-${p.source || 'p'}`} project={p} />)}
          </div>
        )}
      </div>
    </InmobiliariaLayout>
