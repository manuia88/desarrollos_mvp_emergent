          ...
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
              <div>
                ...
                <div style={{ fontFamily: 'DM Sans', fontSize: 11.5, color: 'rgba(240,235,224,0.45)', marginTop: 3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 380 }}>
              ...
              <>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10, marginBottom: 18 }}>
                  {[
                    ...
                    <div key={l} style={{ padding: '12px 14px', borderRadius: 11, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)' }}>
                      <div style={{ fontFamily: 'DM Sans', fontSize: 10.5, color: 'rgba(240,235,224,0.45)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 3 }}>{l}</div>
                ...
                {data.items_pending_review > 0 && (
                  <button data-testid="job-bulk-approve" onClick={() => onBulkApprove(data.id)}
                    style={{ width: '100%', padding: '10px 0', borderRadius: 9999, background: 'linear-gradient(90deg,#6366F1,#EC4899)', border: 'none', color: '#fff', fontFamily: 'DM Sans', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
            ...
            {tab === 'items' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {(data.last_items || []).length === 0 ? (
                  <div style={{ padding: 20, textAlign: 'center', color: 'rgba(240,235,224,0.40)', fontFamily: 'DM Sans', fontSize: 12 }}>Sin items aún</div>
                  ...
                  <div key={it.id} style={{ padding: '8px 12px', borderRadius: 9, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
                    ...
                    <div style={{ fontFamily: 'DM Mono, monospace', fontSize: 10.5, color: 'rgba(240,235,224,0.50)', display: 'flex', gap: 8 }}>
      ...
      <div data-testid="superadmin-bulk-ingest">
        ...
        {stats && (
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 22 }}>
            <KpiCard Icon={Activity} label="Jobs totales" value={stats.jobs_total} accent="#818CF8" />
            ...
            <KpiCard Icon={DollarSign} label="AI mes (MXN)" value={fmtMxn(stats.ai_cost_mes_mxn)} accent="#EC4899" />
          </div>
        )}
        ...
        <div style={{ padding: '16px 18px', borderRadius: 14, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', marginBottom: 22 }}>
          <div style={{ fontFamily: 'Outfit', fontWeight: 700, fontSize: 14, color: 'var(--cream)', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 6 }}>
            <Upload size={13} color="#818CF8" /> Iniciar nueva ingesta
          </div>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            <button data-testid="ingest-start-btn" onClick={handleStart} disabled={busyStart || !form.url.trim()}
              style={{ padding: '9px 20px', borderRadius: 9999, background: 'linear-gradient(90deg,#6366F1,#EC4899)', border: 'none', color: '#fff', fontFamily: 'DM Sans', fontWeight: 700, fontSize: 13, cursor: busyStart ? 'wait' : 'pointer', opacity: busyStart ? 0.7 : 1 }}>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 16, flexWrap: 'wrap' }}>
          <button data-testid="tab-jobs" onClick={() => setTab('jobs')}
            style={{ padding: '8px 14px', borderRadius: 9999, fontSize: 12.5, fontFamily: 'DM Sans', fontWeight: 600, cursor: 'pointer', border: tab === 'jobs' ? '1px solid rgba(99,102,241,0.55)' : '1px solid rgba(255,255,255,0.10)', background: tab === 'jobs' ? 'rgba(99,102,241,0.16)' : 'transparent', color: tab === 'jobs' ? '#818CF8' : 'rgba(240,235,224,0.55)' }}>
            Jobs históricos
          </button>
          <button data-testid="tab-review" onClick={() => setTab('review')}
            style={{ padding: '8px 14px', borderRadius: 9999, fontSize: 12.5, fontFamily: 'DM Sans', fontWeight: 600, cursor: 'pointer', border: tab === 'review' ? '1px solid rgba(99,102,241,0.55)' : '1px solid rgba(255,255,255,0.10)', background: tab === 'review' ? 'rgba(99,102,241,0.16)' : 'transparent', color: tab === 'review' ? '#818CF8' : 'rgba(240,235,224,0.55)', display: 'flex', alignItems: 'center', gap: 6 }}>
            Cola revisión
        ...
        {tab === 'jobs' && (
          <>
            <div style={{ display: 'flex', gap: 6, marginBottom: 14, flexWrap: 'wrap' }}>
              {STATUS_CHIPS.map(([k, l]) => (
                <button key={k} data-testid={`status-chip-${k}`} onClick={() => setStatusFilter(k)}
                  style={{ padding: '5px 12px', borderRadius: 9999, fontSize: 11.5, fontFamily: 'DM Sans', fontWeight: 600, cursor: 'pointer', border: statusFilter === k ? '1px solid rgba(99,102,241,0.45)' : '1px solid rgba(255,255,255,0.08)', background: statusFilter === k ? 'rgba(99,102,241,0.10)' : 'transparent', color: statusFilter === k ? '#818CF8' : 'rgba(240,235,224,0.45)' }}>{l}</button>
              ))}
            ...
            {(jobs.items || []).length === 0 ? (
              <div data-testid="jobs-empty" style={{ padding: 50, textAlign: 'center', color: 'rgba(240,235,224,0.40)', fontFamily: 'DM Sans' }}>
        ...
        {tab === 'review' && (
          <>
            {(reviewItems.items || []).length === 0 ? (
              <div data-testid="review-empty" style={{ padding: 50, textAlign: 'center', color: 'rgba(240,235,224,0.40)', fontFamily: 'DM Sans' }}>
