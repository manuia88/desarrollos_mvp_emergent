                    ...
                    {editProj && editProj.id === p.id ? (
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                        ...
                        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                          ...
                          <button onClick={() => nav(`/superadmin/expediente/${p.id}`)} style={btn(true)} data-testid={`proj-ficha-${p.id}`}><ExternalLink size={12} style={{ verticalAlign: -2, marginRight: 3 }} />Ficha completa</button>
                        </div>
                      </div>
                      {openData === p.id && (
