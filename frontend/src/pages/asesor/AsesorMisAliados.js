...
function CardKpi({ Icon, label, value, color }) {
  return (
    <div style={{ flex: '1 1 0', padding: '8px 10px', borderRadius: 9, background: 'var(--surface-2)', border: '1px solid var(--border)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 3 }}>
        <Icon size={10} color={color || 'var(--cream-3)'} />
        <span style={{ fontFamily: 'DM Sans', fontSize: 9.5, color: 'var(--cream-3)', textTransform: 'uppercase', letterSpacing: '0.07em' }}>{label}</span>
      </div>
    ...
    >
      ...
      <div style={{ padding: '16px 16px 0', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {branding.tagline && (
            <div style={{ fontFamily: 'DM Sans', fontSize: 11.5, color: 'var(--cream-3)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
              {branding.tagline}
            </div>
          )}
        </div>
      ...
      <div style={{ padding: '8px 16px 0', display: 'flex', gap: 6, flexWrap: 'wrap' }}>
        <span style={{ padding: '2px 9px', borderRadius: 9999, fontSize: 10.5, background: 'var(--surface-2)', border: '1px solid var(--border)', color: 'var(--border)', fontFamily: 'DM Sans' }}>
      ...
      <div style={{ width: '100%', maxWidth: 480, background: 'rgba(13,17,28,0.96)', borderLeft: '1px solid var(--border)', padding: '28px 28px 80px', overflowY: 'auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 22 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            {b.tagline && <div style={{ fontFamily: 'DM Sans', fontSize: 12.5, color: 'var(--cream-3)', marginTop: 2 }}>{b.tagline}</div>}
          </div>
          {aliado.commission_pct != null && (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10, marginBottom: 22 }}>
          {[
            ...
            <div key={l} style={{ padding: '12px 14px', borderRadius: 11, background: 'var(--surface-2)', border: '1px solid var(--border)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 3 }}>
                <Ic size={10} color={c} />
                <span style={{ fontFamily: 'DM Sans', fontSize: 10.5, color: 'var(--cream-3)', textTransform: 'uppercase', letterSpacing: '0.07em' }}>{l}</span>
              </div>
        ...
        <div style={{ marginBottom: 16, padding: '12px 14px', borderRadius: 10, background: 'rgba(99,102,241,0.06)', border: '1px solid rgba(99,102,241,0.20)' }}>
          <div style={{ fontFamily: 'DM Sans', fontSize: 11, color: 'var(--cream-3)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 4 }}>Inventario disponible</div>
      ...
      <div data-testid="asesor-mis-aliados" style={{ maxWidth: 1200 }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 18, flexWrap: 'wrap', alignItems: 'center' }}>
          <div style={{ position: 'relative', minWidth: 240, flex: '1 1 240px', maxWidth: 360 }}>
            <Search size={13} color="var(--cream-3)" style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)' }} />
            <input data-testid="aliados-search" value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar desarrollador…"
              style={{ width: '100%', padding: '8px 12px 8px 32px', borderRadius: 9999, background: 'var(--surface-2)', border: '1px solid var(--border)', color: 'var(--cream)', fontFamily: 'DM Sans', fontSize: 12.5, outline: 'none', boxSizing: 'border-box' }} />
          </div>
            ...
            {[['', 'Todas'], ['lt5', '<5%'], ['5to8', '5-8%'], ['gte8', '≥8%']].map(([k, l]) => (
              <button key={k || 'all'} data-testid={`comm-filter-${k || 'all'}`}
                ...
                style={{
                  ...
                  border: commFilter === k ? '1px solid rgba(99,102,241,0.55)' : '1px solid var(--border)',
                  background: commFilter === k ? 'rgba(99,102,241,0.16)' : 'transparent',
                  color: commFilter === k ? '#818CF8' : 'var(--cream-3)',
                }}>{l}</button>
            ))}
          </div>
        </div>
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: 70, color: 'var(--cream-3)', fontFamily: 'DM Sans', fontSize: 13 }}>Cargando aliados…</div>
        ) : filtered.length === 0 ? (
          <div data-testid="aliados-empty" style={{ textAlign: 'center', padding: 70, color: 'var(--cream-3)', fontFamily: 'DM Sans' }}>
