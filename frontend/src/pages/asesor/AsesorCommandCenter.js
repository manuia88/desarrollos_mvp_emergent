...
const probPill = (p) => {
  ...
  return 'text-[var(--cream-3)] bg-[var(--surface-2)] border-[var(--border)]';
};
  ...
  return (
    <div className="animate-pulse space-y-4" data-testid="command-center-skeleton">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[0, 1, 2, 3].map((i) => <div key={i} className="h-24 rounded-2xl bg-[var(--surface-2)]" />)}
      </div>
      <div className="space-y-2">
        {[0, 1, 2].map((i) => <div key={i} className="h-16 rounded-xl bg-[var(--surface-2)]" />)}
      </div>
    </div>
  );
}
      ...
      <div className="max-w-[1100px] mx-auto" data-testid="asesor-command-center">
        {loading ? <Skeleton /> : (
          <>
                    ...
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-3 h-9 rounded-full text-sm font-medium transition-colors ${
                        qa.primary
                          ...
                          : 'text-[var(--cream)] bg-[var(--surface-2)] border border-[var(--border)] hover:bg-[var(--surface-2)]'
                      }`}
                    >
            ...
            {customizing && (
              <div data-testid="customize-panel" className="mb-6 rounded-2xl border border-[var(--border)] bg-[var(--surface-2)] p-4">
                ...
                <div className="space-y-1.5">
                  {(widgetCfg.order || DEFAULT_WIDGETS).map((id, idx, arr) => {
                    ...
                    return (
                      <div key={id} data-testid={`widget-row-${id}`} className="flex items-center gap-2 px-2 py-1.5 rounded-lg bg-[var(--surface-2)]">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
              ...
              <section className="lg:col-span-2">
                ...
                {!queueCollapsed && (
                  queue.length === 0 ? (
                    <div className="flex flex-col items-center justify-center text-center py-12 rounded-2xl border border-dashed border-[var(--border)]" data-testid="queue-empty">
                        ...
                        {archived.length === 0 ? (
                          ...
                          >
                            ...
                            <button
                              ...
                              className="shrink-0 px-2 py-0.5 rounded-md text-[10px] font-medium text-[var(--cream-2)] hover:text-[var(--cream)] hover:bg-[var(--surface-2)] transition-colors"
                            >
                              {t('queue.restore')}
                            </button>
                          </div>
