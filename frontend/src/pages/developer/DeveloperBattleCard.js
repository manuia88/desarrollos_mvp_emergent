  ...
  return (
    ...
    >
      <span style={{
        ...
        fontSize: 10, color: 'rgba(var(--cream-rgb),0.45)',
...
export default function DeveloperBattleCard({ user, onLogout }) {
  ...
  useEffect(() => {
    if (project_id) {
      ...
      setLoading(false);   // sin proyecto seleccionado → no quedar girando
    }
  ...
  if (tierError) {
    return (
      ...
      <div data-testid="battle-card-tier-error" style={{
        ...
        <h2 style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 800, fontSize: 22, color: 'var(--cream)', margin: 0 }}>
          {t('battle_card.errors.tier_required', { defaultValue: 'Battle Card requiere tier T3+' })}
        </h2>
        <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 13, color: 'rgba(var(--cream-rgb),0.5)', margin: 0, maxWidth: 380 }}>
  ...
  // ── Sin proyecto seleccionado (evita spinner infinito en /battle-card) ──
  if (!project_id) {
    return (
      <DeveloperLayout user={user} onLogout={onLogout}>
      <div data-testid="battle-card-no-project" style={{
        minHeight: '60vh',
        display: 'flex', flexDirection: 'column', alignItems: 'center',
        justifyContent: 'center', padding: '40px 24px', gap: 16, textAlign: 'center',
      }}>
        <h2 style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 800, fontSize: 22, color: 'var(--cream)', margin: 0 }}>
          Elige un proyecto para ver su Battle Card
        </h2>
        <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 13, color: 'rgba(var(--cream-rgb),0.5)', margin: 0, maxWidth: 400 }}>
          La Battle Card compara tu proyecto contra la competencia de su zona. Ábrela desde un proyecto.
        </p>
        <button onClick={() => navigate('/desarrollador/proyectos')} style={{
          padding: '11px 26px', borderRadius: 9999, background: `linear-gradient(90deg, ${INDIGO}, ${ROSE})`,
          border: 'none', cursor: 'pointer', color: '#fff', fontFamily: 'DM Sans, sans-serif', fontWeight: 700, fontSize: 14,
        }}>
          Ir a Mis Proyectos →
        </button>
      </div>
    ...
    return (
      ...
      <div data-testid="battle-card-insufficient" style={{
        ...
        <div style={{
          ...
          <h2 style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 800, fontSize: 20, color: 'var(--cream)', marginBottom: 10 }}>
            {t('battle_card.empty.insufficient_competitors_title', {
              defaultValue: 'Battle Card en preparacion',
            })}
          </h2>
          <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 13, color: 'rgba(var(--cream-rgb),0.5)', lineHeight: 1.55, margin: 0 }}>
          ...
          <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 11, color: 'rgba(var(--cream-rgb),0.3)', marginTop: 14 }}>
    ...
    >
      ...
      <div style={{
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
            <span style={{ fontFamily: 'DM Sans', fontSize: 11, color: 'rgba(var(--cream-rgb),0.4)', letterSpacing: '0.08em', textTransform: 'uppercase' }}>
              {data.dev_org_id || ''}
            </span>
            <ChevronRight size={12} color="rgba(var(--cream-rgb),0.3)" />
            <span style={{ fontFamily: 'DM Sans', fontSize: 11, color: 'rgba(var(--cream-rgb),0.4)', letterSpacing: '0.08em', textTransform: 'uppercase' }}>
              {data.zone_slug || ''}
            </span>
          </div>
          <h1 style={{
            fontFamily: 'Outfit, sans-serif', fontWeight: 800,
            fontSize: 26, color: 'var(--cream)', margin: 0, lineHeight: 1.2,
          }}>
            {t('battle_card.title.main', { defaultValue: 'Battle Card' })}
            <span style={{ color: INDIGO }}> · {data.project_name || project_id}</span>
          </h1>
          <p style={{ margin: '4px 0 0', fontSize: 12, color: 'rgba(var(--cream-rgb),0.4)' }}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          <button
            ...
            style={{
              ...
              color: 'rgba(var(--cream-rgb),0.7)', cursor: 'pointer',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 20 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {/* Score gauge */}
          <div style={{
            borderRadius: 18, background: 'rgba(var(--cream-rgb),0.035)',
            border: '1px solid rgba(var(--cream-rgb),0.08)', padding: '6px 0',
          }}>
          ...
          <div style={{
            ...
            <p style={{ margin: 0, fontSize: 10, color: 'rgba(var(--cream-rgb),0.4)', textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: 700 }}>
              {t('battle_card.ranking.position_label', { defaultValue: 'Posicion en zona' })}
            </p>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 4, marginTop: 6 }}>
              <span style={{ fontFamily: 'Outfit', fontWeight: 800, fontSize: 32, color: 'var(--cream)', lineHeight: 1 }}>
                {rankN ? `#${rankN}` : '—'}
              </span>
              {totalZone && (
                <span style={{ fontSize: 11, color: 'rgba(var(--cream-rgb),0.4)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          {competitors.length > 0 && (
            <div style={{
              ...
              <p style={{
                ...
                color: 'rgba(var(--cream-rgb),0.5)', textTransform: 'uppercase', letterSpacing: '0.08em',
              }}>
          ...
          {action && (
            ...
            >
              <p style={{
                ...
                {data.weakest_dim && (
                  <span style={{ color: 'rgba(var(--cream-rgb),0.4)', fontWeight: 600, marginLeft: 6 }}>
                    · {t(`battle_card.dimensions.${data.weakest_dim}`, { defaultValue: data.weakest_dim })}
                  </span>
                )}
              </p>
              <p style={{ margin: '0 0 14px', fontSize: 13, color: 'var(--cream)', lineHeight: 1.55 }}>
                {action}
              </p>
          ...
          <p style={{
            ...
            color: 'rgba(var(--cream-rgb),0.25)', fontFamily: 'DM Sans',
          }}>
