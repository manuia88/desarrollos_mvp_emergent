...
function KpiCell({ label, value, accent }) {
  return (
    <div style={{ minWidth: 80 }}>
      <div style={{ fontFamily: 'DM Sans', fontSize: 10.5, color: 'rgba(240,235,224,0.42)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 2 }}>
        {label}
      </div>
...
function Avatar({ src, name, type }) {
  ...
  return (
    <div style={{
      ...
      {!src && (
        <span style={{ fontFamily: 'Outfit', fontWeight: 800, fontSize: 14, color: '#818CF8' }}>{initial}</span>
      )}
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 200 }}>
        ...
        {subtitle && (
          <div style={{ fontFamily: 'DM Sans', fontSize: 12, color: 'rgba(240,235,224,0.45)' }}>{subtitle}</div>
        )}
      </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
          ...
          <button onClick={onClose} data-testid="drawer-close"
            style={{ padding: '6px 14px', borderRadius: 9999, background: 'transparent', border: '1px solid rgba(255,255,255,0.12)', color: 'rgba(240,235,224,0.55)', fontFamily: 'DM Sans', fontSize: 12, cursor: 'pointer' }}>
            Cerrar
          </button>
        </div>
        ...
        {(entity.assigned_projects || []).length > 0 && (
          <div style={{ marginBottom: 16 }}>
            <div style={{ fontFamily: 'DM Sans', fontSize: 11, color: 'rgba(240,235,224,0.50)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 6 }}>Proyectos asignados</div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {entity.assigned_projects.map(pid => (
                <span key={pid} style={{ padding: '3px 9px', borderRadius: 9999, fontSize: 11, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.10)', color: 'rgba(240,235,224,0.65)', fontFamily: 'DM Sans' }}>{pid}</span>
              ))}
            </div>
          </div>
        )}
    ...
    <DeveloperLayout user={user} onLogout={onLogout}>
      <div data-testid="dev-red-comercial" style={{ maxWidth: 1100 }}>
        ...
        {loading ? (
          ...
          <div data-testid="red-empty" style={{ textAlign: 'center', padding: 70, color: 'rgba(240,235,224,0.40)', fontFamily: 'DM Sans' }}>
