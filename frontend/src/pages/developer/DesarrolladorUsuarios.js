...
const STATUS_CFG = {
  active: { label: 'Activo', color: '#fff', bg: '#1FA06A', bd: '#1FA06A' },
  pending_invitation: { label: 'Invitacion pendiente', color: '#fff', bg: '#C77F12', bd: '#C77F12' },
  pending: { label: 'Invitacion pendiente', color: '#fff', bg: '#C77F12', bd: '#C77F12' },
  suspended: { label: 'Suspendido', color: '#fff', bg: '#E0463D', bd: '#E0463D' },
};
...
function InviteModal({ onClose, onSuccess }) {
  ...
  return (
    <div onClick={e => { if (e.target === e.currentTarget) onClose(); }} style={{ position: 'fixed', inset: 0, background: 'rgba(var(--bg-rgb),0.82)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: Z.MODAL, padding: 16 }}>
      <div style={{ background: 'rgba(var(--bg-rgb),0.97)', border: '1px solid rgba(var(--cream-rgb),0.10)', borderRadius: 18, width: '100%', maxWidth: 440, padding: '26px 26px 22px' }}>
        ...
        <div style={{ marginBottom: 13 }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <ChevronDown size={13} color="rgba(var(--cream-rgb),0.38)" style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' }} />
          </div>
        </div>
      ...
      <div data-testid="desarrollador-usuarios" style={{ maxWidth: 880 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 22, flexWrap: 'wrap', gap: 12 }}>
          <div>
            ...
            <p style={{ fontFamily: 'DM Sans', fontSize: 13, color: 'rgba(var(--cream-rgb),0.50)', margin: 0 }}>Usuarios internos de tu organización.</p>
          </div>
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: 60, color: 'rgba(var(--cream-rgb),0.40)', fontFamily: 'DM Sans', fontSize: 13 }}>Cargando equipo…</div>
        ) : items.length === 0 ? (
          <div data-testid="usuarios-empty" style={{ textAlign: 'center', padding: 60, color: 'rgba(var(--cream-rgb),0.40)', fontFamily: 'DM Sans' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {items.map(u => (
              <div key={u.email} data-testid={`user-row-${u.email}`}
                style={{ padding: '14px 18px', borderRadius: 12, background: 'rgba(var(--cream-rgb),0.03)', border: '1px solid rgba(var(--cream-rgb),0.07)', display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
                <div style={{ width: 38, height: 38, borderRadius: '50%', background: 'linear-gradient(135deg,rgba(99,102,241,0.22),rgba(236,72,153,0.12))', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid rgba(var(--cream-rgb),0.09)', flexShrink: 0 }}>
                  <span style={{ fontFamily: 'Outfit', fontWeight: 800, fontSize: 14, color: 'var(--blue)' }}>{(u.name || u.email || 'U').charAt(0).toUpperCase()}</span>
                </div>
                <div style={{ flex: 1, minWidth: 160 }}>
                  <div style={{ fontFamily: 'Outfit', fontWeight: 700, fontSize: 14, color: 'var(--cream)', marginBottom: 2 }}>{u.name || u.email}</div>
                  <div style={{ fontFamily: 'DM Sans', fontSize: 12, color: 'rgba(var(--cream-rgb),0.45)' }}>{u.email}</div>
                </div>
                <div style={{ display: 'flex', gap: 7, alignItems: 'center', flexWrap: 'wrap' }}>
                  ...
                  {(u.assigned_projects || []).length > 0 && (
                    <span style={{ fontFamily: 'DM Sans', fontSize: 11, color: 'rgba(var(--cream-rgb),0.38)' }}>{u.assigned_projects.length} proyectos</span>
                  )}
                </div>
                <div style={{ display: 'flex', gap: 6 }}>
                  {(u.status === 'pending_invitation' || u.status === 'pending') && u.invitation_id && (
                    <button data-testid={`resend-btn-${u.email}`} onClick={() => handleResend(u.invitation_id)}
                      style={{ padding: '6px 11px', borderRadius: 9999, background: 'rgba(99,102,241,0.09)', border: '1px solid rgba(99,102,241,0.22)', color: 'var(--blue)', fontFamily: 'DM Sans', fontSize: 11.5, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
                      <Mail size={11} /> Reenviar
                    </button>
                  )}
                  {u.status === 'active' && (
                    <button data-testid={`suspend-btn-${u.email}`} onClick={() => handleSuspend(u.email)} disabled={busy[u.email]}
                      style={{ padding: '6px 11px', borderRadius: 9999, background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.22)', color: 'var(--red)', fontFamily: 'DM Sans', fontSize: 11.5, cursor: busy[u.email] ? 'wait' : 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
