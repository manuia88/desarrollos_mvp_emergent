"""W5.ASR.5 Parte 1 — Parser Inmuebles24.

Detecta y extrae datos de leads provenientes de notificaciones Inmuebles24.
"""
...
class Inmuebles24Parser:
    parser_name = "inmuebles24"

    # Señales que identifican un email de Inmuebles24
    _FROM_SIGNALS = (
        "noresponder@inmuebles24.com",
        "noreply@inmuebles24.com",
        "contacto@inmuebles24.com",
        "leads@inmuebles24.com",
    )
    _DOMAIN_SIGNAL = "inmuebles24.com"
    ...
    def match(self, raw_email: dict) -> bool:
        ...
        body = (
            raw_email.get("html") or raw_email.get("text") or ""
        ).lower()
        ...
        if self._DOMAIN_SIGNAL in from_h:
            return True
        # Subject + body combinado
        if "nuevo contacto" in subject and self._DOMAIN_SIGNAL in (body + subject):
            return True
    ...
    def extract(self, raw_email: dict) -> Optional[Dict[str, Any]]:
        ...
        # Conservador: si no hay email NI teléfono, descartamos
    ...
    def _extract_listing_id(self, content: str) -> Optional[str]:
        patterns = [
            r"inmuebles24\.com/[^/]+/[^/]+/(\d{5,12})",
            r"inmuebles24\.com/[^/]+/(\d{5,12})(?:\?|#|$|\.html)",
