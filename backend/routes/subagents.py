...
# ═══════════════════════════════════════════════════════════════════════════════
# W4.5 Y.2C — Lead Sub-Agent endpoints
# ═══════════════════════════════════════════════════════════════════════════════

class AnalyzeFunnelIn(BaseModel):
    period_days: Optional[int] = 30


# ─── POST /api/subagents/lead/analyze ─────────────────────────────────────────
@router.post("/lead/analyze", status_code=201)
async def analyze_lead_funnel(body: AnalyzeFunnelIn, request: Request):
    user = await _require_developer_or_admin(request)
    org_id = getattr(user, "tenant_id", None) or getattr(user, "org_id", None)
    if not org_id:
        raise HTTPException(400, "tenant_id no disponible en sesión")
    ...
    try:
        ...
        result = await agent.analyze_funnel(period_days=body.period_days or 30)
...
# ─── GET /api/subagents/lead/recommendations ──────────────────────────────────
@router.get("/lead/recommendations")
async def list_lead_recommendations(
    request: Request,
    status: Optional[str] = None,
    ...
    limit = min(limit, 200)
    cursor = db.lead_recommendations.find(query).sort("generated_at", -1).skip(skip).limit(limit)
    docs = await cursor.to_list(length=limit)
    total = await db.lead_recommendations.count_documents(query)
...
# ─── POST /api/subagents/lead/recommendations/{id}/apply ──────────────────────
@router.post("/lead/recommendations/{rec_id}/apply")
async def apply_lead_recommendation(rec_id: str, request: Request):
    user = await _require_developer_or_admin(request)
    org_id = getattr(user, "tenant_id", None) or getattr(user, "org_id", None)
    user_id = getattr(user, "user_id", None)

    db = request.app.state.db
    doc = await db.lead_recommendations.find_one({"_id": rec_id})
    if not doc:
        raise HTTPException(404, "Recomendación no encontrada")
    if doc.get("org_id") != org_id and getattr(user, "role", "") != "superadmin":
        raise HTTPException(403, "Acceso denegado")
    ...
    now = datetime.now(timezone.utc)
    await db.lead_recommendations.update_one(
        {"_id": rec_id},
        {"$set": {"status": "applied", "applied_at": now, "applied_by_user_id": user_id}},
    )
    ...
    try:
        ...
        actor_dict = {"user_id": user_id or "unknown", "role": getattr(user, "role", ""), "tenant_id": org_id}
        await log_mutation(
            db, actor_dict, "update", "lead_recommendation",
            entity_id=rec_id,
            before={"status": "pending"},
            after={"status": "applied", "applied_by": user_id},
        )
    except Exception as exc:
        log.warning(f"[routes_subagents] lead audit log failed: {exc}")

    updated = await db.lead_recommendations.find_one({"_id": rec_id})
    return JSONResponse(content=_clean_rec(updated or {}))


# ─── POST /api/subagents/lead/recommendations/{id}/reject ─────────────────────
@router.post("/lead/recommendations/{rec_id}/reject")
async def reject_lead_recommendation(rec_id: str, body: RejectIn, request: Request):
    user = await _require_developer_or_admin(request)
    org_id = getattr(user, "tenant_id", None) or getattr(user, "org_id", None)
    user_id = getattr(user, "user_id", None)
    ...
    if body.reason:
        update_fields["rejection_reason"] = body.reason[:400]
    await db.lead_recommendations.update_one({"_id": rec_id}, {"$set": update_fields})
    ...
    try:
        ...
        log.warning(f"[routes_subagents] lead audit reject failed: {exc}")

    updated = await db.lead_recommendations.find_one({"_id": rec_id})
    return JSONResponse(content=_clean_rec(updated or {}))


# ─── GET /api/superadmin/subagents/lead/runs ──────────────────────────────────
@sa_router.get("/lead/runs")
async def list_lead_runs(
    ...
    db = request.app.state.db
    since = datetime.now(timezone.utc) - timedelta(days=days)
    query: Dict[str, Any] = {"agent_type": "lead", "created_at": {"$gte": since}}
    if org_id:
        query["org_id"] = org_id
