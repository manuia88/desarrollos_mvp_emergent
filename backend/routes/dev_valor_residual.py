...
class CalculoIn(BaseModel):
    ...
    comision_pct_manual: Optional[float] = Field(None, ge=0, le=0.3)
    honorarios_pct_manual: Optional[float] = Field(None, ge=0, le=0.5)
    city: str = "CDMX"
...
async def analizar(request: Request, body: CalculoIn):
    ...
    try:
        return await analizar_lote(
            ...
            eficiencia=body.eficiencia, comision_pct_manual=body.comision_pct_manual,
            honorarios_pct_manual=body.honorarios_pct_manual, city=body.city)
    except Exception as e:
...
async def calcular(request: Request, body: CalculoIn):
    ...
    try:
        return await calcular_residual(
            ...
            comision_pct_manual=body.comision_pct_manual,
            honorarios_pct_manual=body.honorarios_pct_manual,
            city=body.city,
        )
    except Exception as e:
